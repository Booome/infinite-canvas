    ...
    return (
        <div ref={rootRef} className="pointer-events-none absolute bottom-5 z-50 flex justify-center" style={{ left: 300, right: 16 }} data-canvas-bottom-toolbar>
            <div ref={wrapRef} className="thin-scrollbar pointer-events-auto flex h-14 max-w-full items-center gap-1 overflow-x-auto rounded-xl border px-2 shadow-lg backdrop-blur [&>*]:shrink-0" style={dockStyle}>
                ...
                <ToolbarButton id="tool-undo" label={t("canvas.undo")} disabled={!canUndo} hovered={hovered} hoverStyle={hoverStyle} onHover={setHovered} onClick={onUndo}>
                    <Undo2 className="size-4.5" />
                </ToolbarButton>
                <ToolbarButton id="tool-redo" label={t("canvas.redo")} disabled={!canRedo} hovered={hovered} hoverStyle={hoverStyle} onHover={setHovered} onClick={onRedo}>
                    <Redo2 className="size-4.5" />
                </ToolbarButton>
                <ToolbarButton id="tool-focus" label={t("canvas.toolbar.focus")} disabled={!selectedCount} hovered={hovered} hoverStyle={hoverStyle} onHover={setHovered} onClick={onFocusSelected}>
                    <LocateFixed className="size-4.5" />
                </ToolbarButton>
                <ToolbarButton id="tool-focus-back" label={t("canvas.toolbar.focusBack")} disabled={!canFocusBack} hovered={hovered} hoverStyle={hoverStyle} onHover={setHovered} onClick={onFocusBack}>
                    <ChevronLeft className="size-4.5" />
                </ToolbarButton>
                <ToolbarButton id="tool-focus-forward" label={t("canvas.toolbar.focusForward")} disabled={!canFocusForward} hovered={hovered} hoverStyle={hoverStyle} onHover={setHovered} onClick={onFocusForward}>
                ...
                <ToolbarButton id="tool-video" label={t("canvas.toolbar.video")} hovered={hovered} hoverStyle={hoverStyle} onHover={setHovered} onClick={onAddVideo}>
                    <Video className="size-4.5" />
                </ToolbarButton>
                <ToolbarButton id="tool-audio" label={t("canvas.toolbar.audio")} hovered={hovered} hoverStyle={hoverStyle} onHover={setHovered} onClick={onAddAudio}>
                ...
                {extensionDefs.length ? (
                    <ToolbarButton
                        ...
                        onHover={setHovered}
                        onClick={(event) => {
                            setExtPanelX(getAnchorCenterX(wrapRef.current, event.currentTarget));
                            setAppearanceOpen(false);
                ...
                <ToolbarButton id="tool-upload" label={t("canvas.toolbar.upload")} hovered={hovered} hoverStyle={hoverStyle} onHover={setHovered} onClick={onUpload}>
                ...
                <ToolbarButton
                    ...
                    onHover={setHovered}
                    onClick={(event) => {
                        setPanelX(getAnchorCenterX(wrapRef.current, event.currentTarget));
                        setExtensionsOpen(false);
                ...
                <Divider theme={theme} />
                <ToolbarButton id="tool-clear" label={t("canvas.toolbar.clear")} hovered={hovered} hoverStyle={hoverStyle} onHover={setHovered} onClick={onClear} danger>
...
function ToolbarButton({
    ...
    const style = active ? activeStyle : hovered === id && !disabled ? hoverStyle : { color: danger ? "#f87171" : theme.toolbar.item, opacity: disabled ? 0.35 : 1 };
...
function getAnchorCenterX(wrap: HTMLDivElement | null, target: HTMLElement) {
    if (!wrap) return 0;
