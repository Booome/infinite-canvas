...
+ [修复] 「原地生成」下漏判上传素材的问题：素材判定改为「**属性优先 + 标记兜底**」——有内容的节点若**没有任何生成痕迹**（生成类型 / 生成快照 / 模型 / 质量 / 数量 / 文本批次 / 视频任务等只由生成流程写入、且上传替换时会被清掉的字段），就按用户素材处理（打过提示词也算素材）；「参数面板里选过的尺寸」这类**非生成专属**的残留字段不再算痕迹，所以本次修复前上传的图片也能正确隐藏。显式标记（上传/粘贴/插入/手打 → 素材；裁剪/遮罩/分图/放大/帧捕获等编辑产物 → 非素材）优先于属性判断。上传替换时会清空全部生成痕迹（含旧的 `generationSnapshot`），替换后的节点不再自称「生成过」、也不再错误显示重置按钮。节点信息弹窗新增「内容来源 / 生成入口」两行，方便核对判定结果。
+ [修复] 双击图片节点预览时不再连带触发自动居中：双击会把前一次点击已启动的居中动画取消并还原到点击前的视图（预览遮罩同时盖住画布，看不到还原过程），这次没真正到达的聚焦也不计入聚焦历史；节点工具栏的「查看大图」按钮不受影响。单击选中的自动居中保持立即执行，没有延迟。
+ [调整] 加强生成中「禁用生成配置」的视觉反馈：参考栏的「＋」、提示词框（文字变淡 + 内描边）、模型选择、参数设置框、提示词库、展开与重置按钮在禁用时都会明显变淡（重置按钮不可用时图标变灰）；节点快捷工具栏也会临时禁用会改动内容的操作（替换图片/上传替换、遮罩编辑、裁剪、分图、放大、AI 超分、角度、反推提示词、删除节点），禁用项为 35% 透明度并显示禁用光标。**参考列表与提示词里 chip 的跳转（点击聚焦）保持可用，光标仍为手型。**
+ [修复] 修复原地生成后图片节点被缩小的另一处原因：保留尺寸的判定改为「原地生成且节点已有内容」即生效（此前只在节点尺寸不等于默认值时才保留，所以尺寸正好等于默认值的节点——例如刚复制出来的节点——仍会被按生成图重新缩放成很小）。
+ [新增] 新增「错误日志」：画布顶部栏新增日志入口（有记录时显示红点），集中记录全应用错误（各类 `message.error`、未捕获异常与 Promise 拒绝），生成失败还会带上渠道返回的完整报错信息；日志按时间倒序、文本可选中、支持「复制全部」与「清空」，本次会话内保留。生成失败的提示同时改为常驻（需手动关闭），不再几秒后自动消失。
+ [调整] 调整三个默认值：画布默认生成张数由 3 改为 1、「已有内容的节点再次生成时」默认改为「原地生成（覆盖当前节点）」、选中节点默认自动居中。仅影响新装或清空配置的浏览器，已有浏览器保留自己已保存的设置。
+ [修复] 修复「已有内容的节点再次生成」设为「原地生成」后，连生成过的节点也打不开生成菜单的问题：不再用「有没有生成快照」反推「是不是上传素材」（做法改动前生成的旧节点没有快照，会被整体误判），改为在用户真正提供内容时（上传/拖入/粘贴/插入素材/手写文本）显式打标记，只有带标记的节点才隐藏生成入口。
+ [修复] 渠道设置里点「添加渠道」后直接取消（或关掉抽屉），不再残留一个空的「渠道 N」；新渠道改为只在保存时才写入。
+ [新增] 渠道新增「服务商」预设（自定义 / OpenAI / Gemini / HeyRoute / Infistar / PackyCode 等）：选中预设会自动填入渠道名、协议与接口地址并锁定这三项，选「自定义」则全部可自行填写（API Key 与模型列表始终可改）。
+ [新增] 新增 HeyRoute 协议：图片生成与改图按官方要求走 SSE 流式读取（`/v1/images/generations`、`/v1/images/edits`，取 `completed` 事件里的 `b64_json`），视频走 `/v1/videos` 异步任务（创建 → 轮询 → 下载 content，轮询间隔 15 秒）。
+ [调整] 「已有内容的节点再次生成」设为「原地生成」时，上传/粘贴的素材节点（图片、视频、音频、文本）不再出现生成相关入口：快捷工具栏的「复制提示词」「反推提示词」与文本节点的「生成」都会隐藏，节点底部的生成面板也不再打开，避免误把用户素材原地覆盖；此项只在「原地生成」偏好下生效，切回「新建派生节点」即恢复。
+ [调整] 全应用统一悬浮提示（tooltip）：抽出共享的主题化提示组件，88 处提示统一为同一套面板底色、描边、文字与无箭头样式，覆盖画布工具栏/节点/元素列表/各类弹层、顶部导航、Agent 面板、图片工作台等，深色模式下不再出现浅色默认提示。
+ [修复] 底部工具栏此前用的是自绘提示、元素列表与画布节点/顶栏/插件管理/设置/Agent 等 28 处用的是浏览器原生 `title`（既不跟随主题、也和其它提示两套样式），现在全部改为同一组件；深色主题下多选浮出工具栏不再是白底。
+ [调整] 画布元素列表的行与缩略图提示改为跟随鼠标显示（右下偏移），不再盖住相邻行文字；同时合并高频指针事件，每帧最多重新对齐一次，快速移动时更顺滑。
+ [新增] 图片节点面板新增「重置」按钮：生成时记录快照（生成参数、提示词、参考节点），当面板里的参数、提示词或参考与快照不一致时按钮启用，一键恢复为生成这张图时的状态；「复制生成该图片的提示词」与重新生成也改为优先使用同一份快照。
+ [修复] 原地生成（含空图片节点与「原地覆盖」偏好）时，若用户手动改过节点大小，生成成功后不再被重置为默认大小，而是保持当前尺寸。
+ [调整] 进入「添加参考内容」选择模式时，画布加一圈强调色描边与轻微染色、光标改为十字，顶部提示也改用强调色，一眼即可看出正处于选择模式；提示文案同时说明可从左侧元素列表选择。
+ [调整] 画布元素列表的悬浮预览改为靠左对齐、左侧留固定间距，小图更便于观察；预览尺寸上限相应扣除左右间距，避免大图溢出。
+ [修复] 节点快捷工具栏此前硬编码浅色，深色主题下仍是白底；改为跟随画布主题的面板底色、描边与文字色，悬浮提示也改为主题化背景与文字（去掉箭头），深色下文字清晰可见。
+ [新增] 生成过程中锁定生成配置：参考内容的增删、提示词与组装内容编辑、生成模式、模型与各类生成参数都会被禁用，只保留「停止」按钮，避免生成中误改参数。
+ [调整] 生成配置编辑提示词时，Enter 不再触发生成，只用于换行；生成只能点击按钮。`@` 候选菜单打开时 Enter 仍用于选中候选。
+ [新增] 生成配置记住上次用过的参数：每个画布按「图片 / 文本 / 视频 / 音频」分别保存上次的模型、分辨率、比例、张数、质量、视频与音频参数等，新建生成配置时优先使用；切换生成模式也会回填该模式的记忆。记忆随画布一起导出/导入，旧画布无记忆时回退偏好设置的值；提示词、参考内容等输入不参与记忆。
+ [新增] 新增 `desktop/` Tauri 桌面外壳，可把 web 端打包成单文件 exe 双击运行；桌面构建通过 `vite --mode desktop` 隐藏本地 Canvas Agent 入口，网页版行为不变。
+ [新增] 桌面版记住窗口位置、大小和最大化状态，下次启动先恢复再显示窗口，不再从初始位置跳变。
+ [修复] 缩放画布时，节点提示词面板和节点/连线创建菜单不再随画布一起缩放，保持固定的屏幕尺寸。
+ [修复] 「从画布选择参考」模式的提示按钮此前定位在画布坐标原点附近、画布平移后不可见，改为固定在视口顶部居中。
+ [调整] 进入「选择参考」模式时隐藏节点快捷工具栏。
+ [调整] 双击画布和连线创建菜单改为更紧凑的尺寸、字号与间距，宽度收窄为 240px。
+ [修复] 双击画布菜单的节点名称不跟随语言切换，改为实时读取当前语言。
+ [修复] 复制或粘贴节点时，指向该节点的参考连线未一并复制，导致新节点参考栏为空；现在会复制入边参考连线。
+ [新增] 设置新增「已有内容的节点再次生成时」选项，可选择原地覆盖当前节点或新建派生节点。
+ [调整] 该选项默认保持原行为（在已有内容的节点上生成时新建派生节点），需要原地覆盖时可自行切换。
+ [新增] 画布元素列表新增「缩略图」显示类型，可在面板头部切换列表/缩略图，选择会被记住。
+ [新增] 悬浮画布元素列表中的图片/视频节点时，会在画布区域居中显示其预览（按聚焦大小），不遮挡也不拦截列表操作。
+ [新增] 底部工具栏第一组新增「居中聚焦」按钮，聚焦当前选中的节点，与在画布元素列表里点击一致。
+ [新增] 偏好设置新增「选中自动居中」开关，开启后点击节点、生成配置出现时自动把该节点居中。
+ [调整] 「选择参考」模式下，点击画布元素列表里的节点可将其添加为参考，再次点击已添加的则取消参考（已添加的降低亮度标识），而不是居中定位。
+ [新增] 生成配置提示词里的参考图标支持 hover 预览缩略图。
+ [新增] 点击「参考内容」条目或提示词里的参考 chip，可跳转聚焦到对应节点。
+ [新增] 底部工具栏新增聚焦历史的前进/后退按钮；所有触发聚焦的操作都会记录，可在历史间跳转（默认保留 100 条，可在偏好设置配置）。
+ [调整] 偏好设置隐藏滚动条，内容未滚到底时在底部显示提示；「跳转历史条数上限」移到「选中自动居中」上方并改为单行显示。
+ [修复] 统一聚焦落点：聚焦时先打开目标节点面板再计算居中，视图外的目标节点也会先挂载再测量；前进/后退、聚焦按钮和点击画布元素列表首次点击落点即一致，不再偏下。
+ [调整] 点击画布元素列表聚焦节点时同步显示该节点的快捷工具栏，与在画布上点击节点一致。
+ [新增] 画布快捷键：`F` 居中聚焦选中节点，`[` / `]` 在聚焦历史中后退 / 前进（已补进快捷键说明弹窗）。
+ [优化] 快捷键说明弹窗重做版式：按视图 / 选择编辑 / 聚焦 / 媒体分组，统一按键胶囊样式并适配浅色与深色主题；画布顶部与缩放控件两处共用同一份列表。
+ [优化] 统一「参考内容」与提示词 chip 的悬浮预览：共用同一个预览组件，尺寸放大到原来的 2 倍，去掉固定宽度导致的图片留边（宽黑边），浅色/深色下统一使用画布面板底色；两处一致改为「鼠标移出即消失」（不再因移到预览上而保留），上方空间不足时自动翻到下方显示。
+ [优化] 画布节点、参考栏、元素列表与提示词缩略图改用异步解码（`decoding="async"`），减少聚焦跳转时的图片解码阻塞；缩略图不再用 `loading="lazy"`，避免图片拖到光栅阶段才解码造成掉帧。
+ [修复] 从「参考内容」或提示词 chip 跳转聚焦时用的是旧闭包里的画布尺寸，导致节点和面板被挤到上半部分；改为聚焦时实时读取画布尺寸，各入口落点一致。
