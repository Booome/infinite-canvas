...
+ [新增] 生成过程中锁定生成菜单：参考内容的增删、提示词与组装内容编辑、生成模式、模型与各类生成参数都会被禁用，只保留「停止」按钮，避免生成中误改参数。
+ [调整] 生成菜单编辑提示词时，Enter 不再触发生成，只用于换行；生成只能点击按钮。`@` 候选菜单打开时 Enter 仍用于选中候选。+ [新增] 生成菜单记住上次用过的参数：每个画布按「图片 / 文本 / 视频 / 音频」分别保存上次的模型、分辨率、比例、张数、质量、视频与音频参数等，新建生成菜单时优先使用；切换生成模式也会回填该模式的记忆。记忆随画布一起导出/导入，旧画布无记忆时回退偏好设置的值；提示词、参考内容等输入不参与记忆。
...
+ [新增] 偏好设置新增「选中自动居中」开关，开启后点击节点、生成菜单出现时自动把该节点居中。
+ [调整] 「选择参考」模式下，点击画布元素列表里的节点可将其添加为参考，再次点击已添加的则取消参考（已添加的降低亮度标识），而不是居中定位。
+ [新增] 生成菜单提示词里的参考图标支持 hover 预览缩略图。
